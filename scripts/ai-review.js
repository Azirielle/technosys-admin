...
function filterDiff(rawDiff) {
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    if (
      ...
      firstLine.includes('tsconfig.tsbuildinfo') ||
...
Your task is to analyze the git diff of a Pull Request and decide whether to APPROVE it or request changes.

### TECH STACK GUIDELINES:
- Framework: Next.js 16 with App Router.
- Database: Supabase (Server client for auth, service role client only in actions/admin routes).
- UI/Styling: TailwindCSS 4, Lucide React icons.
- Rules: Make sure forms are validated with Zod. Wrap DB actions in try...catch blocks. Verify roles check compliance.
